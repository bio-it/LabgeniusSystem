...
class L6470Stepper():
    ...
    def set_register(self, no, value, signed=False):
        self.interface.xfer_cmd(l6470.SET_PARAM | no)
        bitlen = l6470.bitlen(no)
        self.interface.xfer_param(value, bitlen, signed=signed)
    ...
    def move(self, micro_steps, direction):
        ...
        self.interface.xfer_param(micro_steps, l6470.ABS_POS_BITLEN, signed=True)
    ...
    def run(self, direction, motor_steps_per_sec):
        speed = self._calc_speed(motor_steps_per_sec)
        self.interface.xfer_cmd(l6470.RUN | direction)
    ...
    ###################################
    def go_home(self):
        self.interface.xfer_cmd(l6470.GO_HOME)
    ...
    def is_step_loss(self):
        masked = self.get_status() & l6470.STATUS_BIT_STEP_LOSSES
        return not (masked == l6470.STATUS_BIT_STEP_LOSSES)
    ...
    ###################################
    def soft_hi_z(self):
        self.interface.xfer_cmd(l6470.SOFT_HIZ)
    def hard_hi_z(self):
        # Put the bridges in Hi-Z state immediately with no deceleration.
        self.interface.xfer_cmd(l6470.HARD_HIZ)
